...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/rpa_whatsapp.log"), logging.StreamHandler()],
)
...
class WhatsAppRPAExtractor:
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                context = await browser.new_context(
                    storage_state=(
                        str(self.auth_file) if self.auth_file.exists() else None
                    ),
    ...
    async def _process_ai_summary(self, messages: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 추가 메타데이터
            extraction_data = self.whatsapp_processor.extract_summary_data(
                parsed_messages
            )
    ...
    def _calculate_extraction_confidence(self, messages: List[str]) -> float:
        ...
        # 메시지 품질 점수
        quality_score = sum(1 for msg in messages if len(msg.strip()) > 10) / len(
            messages
        )
        ...
        # 타임스탬프 포함 여부
        timestamp_score = sum(
            1 for msg in messages if any(char.isdigit() for char in msg[:20])
        ) / len(messages)
