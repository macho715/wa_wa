...
class WhatsAppDOMAnalyzer:
    ...
    async def analyze_dom_structure(self):
        ...
        print("🔍 WhatsApp Web DOM 구조 분석 시작")
        print("=" * 50)
    ...
    async def analyze_search_elements(self, page):
        ...
        print(f"✅ 검색 요소 {len(search_elements)}개 발견")
        
        # 발견된 요소들 출력
        for i, element in enumerate(search_elements):
            print(f"  {i+1}. {element['selector']}")
            print(f"     📝 텍스트: {element['text_content']}")
            print(f"     🏷️ aria-label: {element['aria_label']}")
    ...
    async def analyze_chat_elements(self, page):
        ...
        print(f"✅ 채팅방 요소 {len(chat_elements)}개 발견")
    ...
    async def analyze_message_elements(self, page):
        ...
        print(f"✅ 메시지 요소 {len(message_elements)}개 발견")
...
async def main():
    """메인 실행 함수"""
    print("🔍 WhatsApp Web DOM 구조 분석기")
    print("=" * 40)
    print("⚠️ 이 도구는 WhatsApp Web의 실제 DOM 구조를 분석합니다.")
    print("⚠️ QR 코드 스캔이 필요할 수 있습니다.")
    print("=" * 40)
