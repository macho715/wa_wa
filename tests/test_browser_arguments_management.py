...
try:
    ...
    pytest.skip(
        "whatsapp_media_ocr_extractor module not available", allow_module_level=True
    )
...
class TestBrowserArgumentsManagement:
    ...
    def test_should_prevent_duplicate_browser_arguments(self):
        ...
        # When: 중복 제거 메서드 호출
        deduplicated_args = self.extractor._deduplicate_browser_arguments(
            duplicate_args
        )
    ...
    def test_should_use_set_for_argument_deduplication(self):
        ...
        # When: Set 기반 중복 제거
        combined_args = self.extractor._combine_browser_arguments(
            args_list_1, args_list_2
        )
    ...
    def test_should_handle_mixed_case_arguments(self):
        ...
        mixed_case_args = ["--no-sandbox", "--NO-SANDBOX", "--No-Sandbox"]
...
class TestOperationalStability:
    ...
    def test_should_implement_polling_for_browser_status(self):
        ...
        # When: 폴링 수행
        with patch("asyncio.sleep") as mock_sleep:
            result = self.extractor._poll_browser_status(
                mock_page, interval=1, max_attempts=3
            )
