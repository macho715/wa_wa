...
try:
    from whatsapp_media_ocr_extractor import (
        MediaOCRProcessor,
        WhatsAppMediaOCRExtractor,
    )
except ImportError:
    pytest.skip(
...
class TestWhatsAppMediaOCRExtractor:
    ...
    async def test_process_media_file_mock(self):
        """미디어 파일 처리 모의 테스트"""
        with patch.object(
            self.extractor.media_processor, "process_image"
        ) as mock_process:
            ...
            with patch.object(extractor, "download_media") as mock_download:
                with patch.object(extractor, "process_media_file") as mock_process:
                    with patch.object(extractor, "save_results") as mock_save:
                        ...
                        media_elements = await extractor.find_media_messages(
                            mock_page, "Test Chat"
                        )
